...
from component_selector import (
    METHODS, WINDOW, MIN_SAMPLES, breakeven_rate, raw_direction, summarize, select_method,
)
...
class TradingSignalBotV3(previous.TradingSignalBotV3):
        ...
        async with self._decision_send_lock:
            ...
            try:
                ...
                if snapshot is None:
                    ...
                    payout_rate = float(await self.db.get("payout_rate", str(self.config.payout_rate)))
                    selected = select_method(forecast.components, stats, payout_rate=payout_rate)
    ...
    async def threshold_stats_text(self):
        await self._ensure_component_schema()
        payout_rate = float(await self.db.get("payout_rate", str(self.config.payout_rate)))
        breakeven = breakeven_rate(payout_rate)
        return (self._stats_text(await self.component_stats(self.server_now_ms()))
                ...
                + f"\nChỉ chọn khi cận dưới độ tin cậy (Wilson, đã hiệu chỉnh đa so sánh 6×2)"
                + f" của tỷ lệ thắng vượt hòa vốn {breakeven:.1%} (theo tỷ lệ trả thưởng hiện tại)."
                + "\nCó thể đảo hướng gửi khi chiều đảo mới là chiều đạt ngưỡng; không đảo thống kê gốc."
                + "\nKhông đủ bằng chứng thống kê ở phiên nào → KHÔNG MUA phiên đó, không dựa vào kết quả gần nhất.")
