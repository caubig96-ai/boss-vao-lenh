"""Two-method M5 pair matchers.

Both methods only inspect closed candles from the 24 hours before the live M5.
A matched pair must also have an already-closed successor, so forecasts never
read the candle they are trying to predict.
"""
...
def _closed_window(history, before_open_time=None):
    ...
    candles = sorted(
        {
            ...
            and c.close_time < before_open_time
        }.values(),
        key=lambda c: c.open_time,
    )
    return candles, before_open_time


def find_pair_matches(history, before_open_time=None) -> dict[str, PairMatch | None]:
    """Legacy majority matcher retained for frozen V3.7.9/V3.8.0 regressions."""
    candles, before_open_time = _closed_window(history, before_open_time)
    empty = {method: None for method in METHODS}
    if not candles or len(candles) < 5:
        return empty
    if [c.open_time for c in candles[-2:]] != [before_open_time - 600_000, before_open_time - 300_000]:
        return empty
    ...
    for method, occurrences in votes.items():
        ...
        empty[method] = PairMatch(
            method, direction, 1.0, int(a.open_time), int(successor.open_time), green, red
        )
    return empty


def find_nearest_pair_matches(history, before_open_time=None) -> dict[str, PairMatch | None]:
    """Use the most recent matching historical pair instead of a majority vote.

    The live setup is the two latest closed M5 candles. Historical candidates are
    examined inside the preceding 24-hour window. For each method, the most recent
    matching pair whose successor is already closed wins. The forecast direction is
    exactly the color of that single successor candle.
    """
    candles, before_open_time = _closed_window(history, before_open_time)
    empty = {method: None for method in METHODS}
    if not candles or len(candles) < 5:
    ...
    colors = tuple(candle_direction(c) for c in current)
    wicks = tuple(wick_type(c) for c in current)
    nearest: dict[str, tuple | None] = {method: None for method in METHODS}

    # Candles are ascending, so overwriting leaves the closest valid historical pair.
    for i in range(len(candles) - 4):
        a, b, successor = candles[i:i + 3]
        if b.open_time - a.open_time != 300_000 or successor.open_time - b.open_time != 300_000:
            continue
        if (wick_type(a), wick_type(b)) != wicks:
            continue
        nearest["shape_pair"] = (a, successor)
        if (candle_direction(a), candle_direction(b)) == colors:
            nearest["color_pair"] = (a, successor)

    for method, occurrence in nearest.items():
        if occurrence is None:
            continue
        a, successor = occurrence
        direction = candle_direction(successor)
        empty[method] = PairMatch(
            method=method,
            raw_direction=direction,
            similarity=1.0,
            matched_open_time=int(a.open_time),
            matched_next_open_time=int(successor.open_time),
            green_count=1 if direction == "UP" else 0,
            red_count=1 if direction == "DOWN" else 0,
        )
    return empty
...
def select_method(matches: dict[str, PairMatch | None], stats: dict[str, dict]) -> dict | None:
    """Legacy exact color + wick majority selector retained for old versions."""
    ...
    return {
        ...
        "match": match.to_dict(),
        "raw_stats": dict(stats["color_pair"]),
    }
