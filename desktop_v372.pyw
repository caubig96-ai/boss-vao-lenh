...
class VisibleStartupApplication(desktop35.BinanceKlineTrayApplication):
    """Open a visible startup prompt instead of silently hiding in the tray."""

    def __init__(self):
        super().__init__()
        self._startup_error_shown = False
        # Previous builds started hidden in the Windows tray. Make launch visible
        # so the user can immediately tell that the tool actually started.
...
if __name__ == "__main__":
    try:
        ...
        # Windowed PyInstaller otherwise exits with no console and looks like the
        # program never ran. Always surface a final startup error to the user.
        root = tk.Tk()
        root.withdraw()
        messagebox.showerror(
